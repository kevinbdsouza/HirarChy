...
import ln_lstm as lstm
from data_utils import get_data, get_cumpos, convert_indices
import warnings
import random
    ...
    def train_model(self, optimizer):
        ...
        for epoch in tqdm(range(num_epochs)):
            with torch.autograd.set_detect_anomaly(True):
                ...
                random.shuffle(cfg.chr_train_list)
                for chr in cfg.chr_train_list:
                    cum_idx, nrows, cum_pos = get_data(cfg, chr, mode="train")
                    cum_idx = torch.tensor(cum_idx).long().to(device)

                    full_pairs = np.load(cfg.processed_data_dir + "full_pairs_%s.npy" % chr, allow_pickle=True)
                    full_values = np.load(cfg.processed_data_dir + "full_values_%s.npy" % chr, allow_pickle=True)

                    batch_indices = np.arange(0, full_pairs.shape[0])
                    random.shuffle(batch_indices)

                    chr_loss = 0.0
                    for b in batch_indices:
                        batch_pairs = np.array(full_pairs[b])
                        batch_values = np.array(full_values[b])

                        sample_indices = np.random.permutation(batch_pairs.shape[0])
                        batch_pairs, batch_values = torch.tensor(batch_pairs[sample_indices]), torch.tensor(
                            batch_values[sample_indices])

                        batch_pairs = batch_pairs.long().to(device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for chr in cfg.chr_test_list:
                ...
                cum_idx, values, nrows = get_data(cfg, chr, mode="test")
                comp_mat = torch.zeros((nrows + 1, nrows + 1)).to(device)

                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    input_pairs = torch.combinations(cum_idx, with_replacement=True)

                input_pairs = input_pairs.long()
                input_pairs_red = convert_indices(input_pairs, cum_pos)
                input_pairs_red[input_pairs_red < 0] = torch.tensor([0])
                values = values[input_pairs_red[:, 0], input_pairs_red[:, 1]]

                input_pairs = input_pairs.float().to(device)
                values = values.float().to(device)
                cum_idx = cum_idx.to(device)

                "test Pass"
                full_reps = self(cum_idx, nrows)
                error, og_values, pred_values = self.fullMLP(input_pairs.long(), values, cum_pos, full_reps)

                comp_mat[input_pairs_red[:, 0], input_pairs_red[:, 1]] = og_values
                comp_mat[input_pairs_red[:, 1], input_pairs_red[:, 0]] = pred_values
